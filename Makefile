...
PROJECT_NAME = DTCD-ExternalDataSource-OTLRW
PLUGIN_NAME = ExternalDataSource-OTLRW
...
CONNECTOR_URL = $(DEV_STORAGE)/$(CONNECTOR_NAME)/$(CONNECTOR_NAME)-1.2.0-master-0005.tar.gz

.SILENT:

COMPONENTS: sdk connector
...
connector:
	echo $(CONNECTOR_URL)
	echo Downloading $(CONNECTOR_NAME)...
	if ! [ -d ./$(CONNECTOR_NAME) ];\
		then curl -# -Lk $(CONNECTOR_URL) | tar -zx ./$(CONNECTOR_NAME) && echo $(CONNECTOR_NAME) downloaded.;\
		else echo $(CONNECTOR_NAME) is already downloaded.;\
	fi
